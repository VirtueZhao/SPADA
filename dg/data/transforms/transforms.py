...
def _build_transform_train(cfg, choices, target_size, normalize):
    print("+Calling: DDAIG.__init__().SimpleTrainer.__init__().build_data_loader().DataManager.__init__().build_transform()._build_transform_train()")
    tfm_train = []
    ...
    if "randomgrayscale" in choices:
        print("tfm_train += gray scale")
        tfm_train += [RandomGrayscale(p=cfg.INPUT.RGS_P)]

    if "gaussian_blur" in choices:
        print(f"tfm_train += gaussian blur (kernel={cfg.INPUT.GB_K})")
        tfm_train += [RandomApply([GaussianBlur(cfg.INPUT.GB_K)], p=cfg.INPUT.GB_P)]

    print("tfm_train += to torch tensor of range [0, 1]")
    tfm_train += [ToTensor()]

    if "cutout" in choices:
        cutout_n = cfg.INPUT.CUTOUT_N
        cutout_len = cfg.INPUT.CUTOUT_LEN
        print("tfm_train += cutout (n_holes={}, length={})".format(cutout_n, cutout_len))
        tfm_train += [Cutout(cutout_n, cutout_len)]

    if "normalize" in choices:
        print("tfm_train += normalization (mean={}, std={})".format(cfg.INPUT.PIXEL_MEAN, cfg.INPUT.PIXEL_STD))
        tfm_train += [normalize]

    if "gaussian_noise" in choices:
        print("tfm_train += gaussian choice (mean={}, std={})".format(cfg.INPUT.GN_MEAN, cfg.INPUT.GN_STD))
        tfm_train += [GaussianNoise(cfg.INPUT.GN_MEAN, cfg.INPUT.GN_STD)]

    if "instance_norm" in choices:
        print("tfm_train += instance normalization")
        tfm_train += [InstanceNormalization()]

    tfm_train = Compose(tfm_train)
    print("-Closing: DDAIG.__init__().SimpleTrainer.__init__().build_data_loader().DataManager.__init__().build_transform()._build_transform_train()")

    return tfm_train


def _build_transform_test(cfg, choices, target_size, normalize):
    print("+Calling: DDAIG.__init__().SimpleTrainer.__init__().build_data_loader().DataManager.__init__().build_transform()._build_transform_test()")
    tfm_test = []

    interp_mode = INTERPOLATION_MODES[cfg.INPUT.INTERPOLATION]

    print(f"tfm_test += resize the smaller edge to {max(cfg.INPUT.SIZE)}")
    tfm_test += [Resize(max(cfg.INPUT.SIZE), interpolation=interp_mode)]

    print(f"tfm_test += {target_size} center crop")
    tfm_test += [CenterCrop(cfg.INPUT.SIZE)]

    print("tfm_test += to torch tensor of range [0, 1]")
    tfm_test += [ToTensor()]

    if "normalize" in choices:
        print("tfm_test += normalization (mean={}, std={})".format(cfg.INPUT.PIXEL_MEAN, cfg.INPUT.PIXEL_STD))
        tfm_test += [normalize]

    if "instance_norm" in choices:
        print("tfm_test += instance normalization")
        tfm_test += [InstanceNormalization()]

    tfm_test = Compose(tfm_test)
    print("-Closing: DDAIG.__init__().SimpleTrainer.__init__().build_data_loader().DataManager.__init__().build_transform()._build_transform_test()")
