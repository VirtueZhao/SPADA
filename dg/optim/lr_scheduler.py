...
def build_lr_scheduler(optimizer, optim_cfg):
    ...
    if lr_scheduler not in AVAI_SCHEDS:
        raise ValueError("Unsupported scheduler: {}. Must be one of {}".format(lr_scheduler, AVAI_SCHEDS))
    ...
    if lr_scheduler == "single_step":
        ...
        if not isinstance(stepsize, int):
            raise TypeError("For single_step lr_scheduler, stepsize must be an integer, but got {}".format(type(stepsize)))
    ...
    #         raise TypeError("For multi_step lr_scheduler, stepsize must be a list, but got {}".format(type(stepsize)))
    #
